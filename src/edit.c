...
static void make_redo(TEXTP t_ptr)
{
	short redo;
	...
	/*
	 * Restore to undo_y position for redo operations
	 * Ensures consistency with undo behavior
	*/
	t_ptr->cursor_line = get_line(&t_ptr->text,undo_y);
	t_ptr->ypos = undo_y;
	t_ptr->xpos = 0;
...
*/
	...
	{
		for (i=chg_anz; (--i)>=0; )			/* unn�tze �nderung */
			if (chg[i].link==link)
			{
